...
| Task / Dev Task | User Story, Background, Decision (if applicable), Acceptance Criteria, Testing |
...
Write the draft using Markdown. Key formatting rules learned from experience:

- Use `###` (level 3) headings for all sections
...
- Warning/callout: `> ⚠️ **Label** — text` (blockquote with emoji)
- `{warning}` Jira wiki macro does **not** work via CLI
...
### <Open question heading, if needed>
...
- `archived` — it's a reference artifact, not a live note
...
### 9. Update the Obsidian draft note

Add the Jira ticket key and URL to the top of the saved draft note (or its frontmatter) so it's easy to find later.
